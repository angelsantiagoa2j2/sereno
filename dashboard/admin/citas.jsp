...
<%
    ...
    int cntPend=0, cntConf=0, cntCanc=0, cntReal=0;
    try {
        ...
        try {
            // Contadores
            try (PreparedStatement ps = conn.prepareStatement("SELECT estado, COUNT(*) as cnt FROM citas GROUP BY estado")) {
                ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    String e = rs.getString("estado"); int c = rs.getInt("cnt");
                    if ("PENDIENTE".equals(e)) cntPend = c;
                    else if ("CONFIRMADA".equals(e)) cntConf = c;
                    else if ("CANCELADA".equals(e)||"RECHAZADA".equals(e)) cntCanc += c;
                    else if ("REALIZADA".equals(e)) cntReal = c;
                }
            }
            StringBuilder sql = new StringBuilder(
                "SELECT c.id, c.fecha_solicitada, c.estado, c.notas_cliente, " +
                "p.titulo, p.id AS pid, " +
                "uc.nombre AS cnombre, uc.apellido AS capellido, uc.email AS cemail, " +
            ...
            if (!filtro.isEmpty()) sql.append(" WHERE c.estado=?");
            sql.append(" ORDER BY c.id DESC");
            PreparedStatement ps = conn.prepareStatement(sql.toString());
            if (!filtro.isEmpty()) ps.setString(1, filtro);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                String fecha = rs.getString("fecha_solicitada") != null ? rs.getString("fecha_solicitada").substring(0,16) : "";
...
<head>
  ...
  <title>Citas — Sereno Admin</title>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
    ...
    .td-email{font-size:11px;color:var(--slate-lt);}
    ...
    .td-notes{font-size:12px;color:var(--slate-lt);max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
<aside class="sidebar">
  ...
  <nav>
    ...
    <a href="citas.jsp" class="nav-link active"><i class="bi bi-calendar-check"></i> Citas</a>
...
<div class="main">
  ...
  <div class="header-banner">
    <div>
      ...
      <h1 class="hb-title">Gestión de <em>Citas</em></h1>
      <p class="hb-sub"><%= cntPend + cntConf + cntCanc + cntReal %> citas registradas en total</p>
    </div>
  </div>

  <!-- STATS STRIP como filtros -->
  <div class="stats-strip">
    <a href="citas.jsp" class="sstat <%= filtro.isEmpty() ? "active-filter" : "" %>">
      <div class="sstat-icon all"><i class="bi bi-list-ul"></i></div>
      <div><div class="sstat-num"><%= cntPend+cntConf+cntCanc+cntReal %></div><div class="sstat-lbl">Todas</div></div>
    </a>
    <a href="citas.jsp?estado=PENDIENTE" class="sstat <%= "PENDIENTE".equals(filtro) ? "active-filter" : "" %>">
    ...
    <a href="citas.jsp?estado=CONFIRMADA" class="sstat <%= "CONFIRMADA".equals(filtro) ? "active-filter" : "" %>">
      <div class="sstat-icon green"><i class="bi bi-check-circle"></i></div>
      <div><div class="sstat-num"><%= cntConf %></div><div class="sstat-lbl">Confirmadas</div></div>
    </a>
    <a href="citas.jsp?estado=CANCELADA" class="sstat <%= "CANCELADA".equals(filtro) ? "active-filter" : "" %>">
      <div class="sstat-icon red"><i class="bi bi-x-circle"></i></div>
      <div><div class="sstat-num"><%= cntCanc %></div><div class="sstat-lbl">Canceladas</div></div>
    </a>
    <a href="citas.jsp?estado=REALIZADA" class="sstat <%= "REALIZADA".equals(filtro) ? "active-filter" : "" %>">
      <div class="sstat-icon blue"><i class="bi bi-calendar2-check"></i></div>
      <div><div class="sstat-num"><%= cntReal %></div><div class="sstat-lbl">Realizadas</div></div>
    </a>
  </div>
    ...
    <div class="panel">
      <div class="panel-head">
        <div class="panel-title">
          <% if (!filtro.isEmpty()) { %>
            Citas · <span style="font-style:italic;color:var(--blue-bright)"><%= filtro.toLowerCase() %></span>
          <% } else { %>
            Todas las citas
          <% } %>
        </div>
        <span class="panel-count"><%= citas.size() %> resultado<%= citas.size()!=1?"s":"" %></span>
      </div>

      <% if (citas.isEmpty()) { %>
        <div class="empty-state">
          <i class="bi bi-calendar-x"></i>
          No hay citas<%= !filtro.isEmpty() ? " con estado " + filtro.toLowerCase() : "" %>.
        </div>
      <% } else { %>
      <table>
        <thead>
        ...
        <tbody>
          <% for (String[] c : citas) {
              String est = c[2];
              String bc  = "CONFIRMADA".equals(est) ? "badge-confirmed"
                         : "CANCELADA".equals(est)||"RECHAZADA".equals(est) ? "badge-cancelled"
                         : "REALIZADA".equals(est) ? "badge-done" : "badge-pending";
              String bIco = "CONFIRMADA".equals(est) ? "bi-check-circle"
                          : "CANCELADA".equals(est)||"RECHAZADA".equals(est) ? "bi-x-circle"
                          : "REALIZADA".equals(est) ? "bi-calendar2-check" : "bi-hourglass-split";
          %>
          <tr>
            <td><span class="td-id">CIT-<%= c[0] %></span></td>
            <td>
              <div class="td-name"><%= c[6] %></div>
              <div class="td-email"><%= c[7] %></div>
            </td>
            <td>
              <a href="<%= request.getContextPath() %>/propiedades?id=<%= c[5] %>" class="td-prop">
                <i class="bi bi-building" style="font-size:11px;margin-right:3px"></i><%= c[4] %>
              </a>
            </td>
            <td><span class="td-inmo"><%= c[8] %></span></td>
            <td>
              <span class="td-date">
                <i class="bi bi-clock" style="font-size:10px;margin-right:3px"></i><%= c[1] %>
              </span>
            </td>
            <td>
              <span class="td-notes" title="<%= c[3] %>">
                <%= c[3].isEmpty() ? "—" : c[3].length()>45 ? c[3].substring(0,45)+"…" : c[3] %>
              </span>
            </td>
            <td>
              <span class="badge <%= bc %>">
                <i class="bi <%= bIco %>"></i> <%= est %>
              </span>
            </td>
          </tr>
          <% } %>
