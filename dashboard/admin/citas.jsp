...
<head>
    <meta charset="UTF-8"/>
    <title>Citas — InmoVista Admin</title>
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="citas.jsp" class="nav-item active"><i class="bi bi-calendar-check"></i> Citas</a>
...
<div class="main">
    <div class="topbar">
        <span class="topbar-title">Gestión de Citas</span>
    ...
    <div class="content">
        ...
        <div class="card mb-3 p-3">
            <div class="d-flex gap-2 flex-wrap align-items-center">
                ...
                <a href="citas.jsp" class="btn btn-sm <%= filtro.isEmpty()?"btn-dorado":"btn-outline-secondary" %>">Todas (<%= citas.size() %>)</a>
                <a href="citas.jsp?estado=PENDIENTE" class="btn btn-sm <%= "PENDIENTE".equals(filtro)?"btn-warning":"btn-outline-warning" %>">Pendientes</a>
                <a href="citas.jsp?estado=CONFIRMADA" class="btn btn-sm <%= "CONFIRMADA".equals(filtro)?"btn-success":"btn-outline-success" %>">Confirmadas</a>
                <a href="citas.jsp?estado=CANCELADA" class="btn btn-sm <%= "CANCELADA".equals(filtro)?"btn-danger":"btn-outline-danger" %>">Canceladas</a>
                <a href="citas.jsp?estado=REALIZADA" class="btn btn-sm <%= "REALIZADA".equals(filtro)?"btn-info":"btn-outline-info" %>">Realizadas</a>
            </div>
        </div>
        ...
        <div class="card">
            <table class="table table-hover align-middle mb-0">
                ...
                %>
                <tr>
                    <td><small class="text-muted">#<%= c[0] %></small></td>
                    <td>
                        <div class="fw-semibold small"><%= c[6] %></div>
                        <small class="text-muted"><%= c[7] %></small>
                    </td>
                    <td><a href="<%= request.getContextPath() %>/propiedades?id=<%= c[5] %>" class="text-decoration-none small"><%= c[4] %></a></td>
                    <td><small><%= c[8] %></small></td>
                    <td><small><%= c[1] %></small></td>
                    <td><small class="text-muted"><%= c[3].length()>40 ? c[3].substring(0,40)+"..." : c[3].isEmpty()?"—":c[3] %></small></td>
